				...
				for (size_t i = 0; i < 20; i++)
				{
					...
					if (updatemask & stat)
					{
						...
						case Character::MS_SKIN:
							player.getlook().setbody(client.getcache().getequips().getbody(static_cast<int8_t>(recv.readshort())));
							break;
						case Character::MS_FACE:
							player.getlook().setface(client.getcache().getequips().getface(recv.readint()));
							break;
						case Character::MS_HAIR:
							player.getlook().sethair(client.getcache().getequips().gethair(recv.readint()));
							break;
