...
namespace Util
{
	namespace Sha256
	{
		string gethash(const uint8_t* bytes, size_t length)
		{
		...
		uint32_t rotr(uint32_t val, size_t count)
		{
			return (val >> count) | (val << ((sizeof(val) << 3) - count));
		}

		uint32_t rotl(uint32_t val, size_t count)
		{
			return (val << count) | (val >> ((sizeof(val) << 3) - count));
		}

		void process(const uint8_t* bytes, uint32_t* hash)
		{
			...
			static const uint32_t fcbrp[64] =
			{
		...
		void feed(const uint8_t* bytes, size_t length, uint32_t* hash)
		{
		...
		void padbytes(uint8_t* buffer, size_t length)
		{
		...
		void feedremains(const uint8_t* bytes, size_t length, uint32_t* hash)
		{
			// Process data in 64-byte chunks.
			size_t offset = 0;
